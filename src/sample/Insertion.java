...
public class Insertion implements Initializable {
    ...
    public void save(ActionEvent actionEvent) throws SQLException {
        ...
        cn = MV.getConnection();
        String query = "INSERT INTO movies(Movies,Date,Time,Seats,Screen,SeatClass) VALUES ('"+MovieT.getText()+"','"+Dates.getValue().toString()+"','"+Timings.getText()+"','"+no_seats.getText()+"','"+Screen_no.getText()+"','"+Seat_Classes.getText()+"')";
        Statement state = cn.createStatement();
        state.executeUpdate(query);

        Alert a = new Alert(Alert.AlertType.CONFIRMATION);
        a.setTitle("Success");
        a.setContentText("Confirmed");
        a.show();
